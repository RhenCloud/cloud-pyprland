    ...
    async def event_activewindowv2(self, _addr: str) -> None:
        ...
        for client in clients:
            if client["address"] == _addr:
                ...
                self.log.debug(
                    "fcitx5_switcher: active client class=%s title=%s", cls, title
                )
                ...
                should_enable = matches_any(cls, active_classes) or matches_any(
                    title, active_titles
                )
                should_disable = matches_any(cls, inactive_classes) or matches_any(
                    title, inactive_titles
                )

                if should_enable:
                    self.log.debug(
                        "fcitx5_switcher: enabling fcitx for class=%s title=%s",
                        cls,
                        title,
                    )
                ...
                if should_disable:
                    self.log.debug(
                        "fcitx5_switcher: disabling fcitx for class=%s title=%s",
                        cls,
                        title,
                    )
